...
def _load_habits(tool_context: ToolContext) -> list:
    """Load habits list from ADK session state, defaulting to empty list."""
    return tool_context.state.get("habits", [])


def add_habit(tool_context: ToolContext, name: str, frequency: str, start_date: str, goal: str) -> dict:
    ...
    Returns:
        On success: {"status": "ok", "habit_id": str, "name": str}
    ...
    habits = _load_habits(tool_context)

    existing_ids = {h["id"] for h in habits}
    base_id = f"{name.strip()[:3].lower()}-{datetime.date.today().strftime('%Y%m%d')}"
    habit_id = base_id
    counter = 1
